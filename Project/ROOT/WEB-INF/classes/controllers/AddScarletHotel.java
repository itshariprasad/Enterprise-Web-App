...
@WebServlet("/addScarletHotel")
@MultipartConfig(fileSizeThreshold=1024*1024*2, 
				maxFileSize=1024*1024*10,
				maxRequestSize=1024*1024*50)
public class AddScarletHotel extends HttpServlet{
	...
	public void doPost(HttpServletRequest request,HttpServletResponse response) throws IOException
	{
		...
		while (itr.hasNext()) 
		{
			...
			user1.inserthotel(hotelName, city, zip, address, url, typeCount, description, request,response);
		}
		session.setAttribute("hotelImages", imagesLocation);
		response.sendRedirect("/CSPprojectTeam5/addRooms.jsp");
	}
}
